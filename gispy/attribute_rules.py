...
def get_rules(feature) -> list:
    print(f"\nGetting Attribute Rules for '{feature}'...")

    rules = arcpy.Describe(feature).attributeRules
...
def add_sequence_rule(workspace, feature_name, field_name, sequence_prefix="", padded_sequence=False, start_value=1,
    ...
    sde_sequences_sql = """
      ...
      WHERE SEQUENCE_NAME NOT LIKE '%$%' AND SEQUENCE_NAME NOT LIKE 'R[0-9][0-9][0-9][0-9][0-9][0-9]'
