...
def sync_replicas(replica_name: str, rw_sde: str, ro_sde: str):
    ...
    arcpy.SynchronizeChanges_management(
        ...
        # reconcile="RECONCILE "  # TODO: Should this space be removed..? Fixed in Pro 3.x
        reconcile = "RECONCILE"
    )
...
def add_to_replica(
        replica_name: str, rw_sde: str, ro_sde: str, add_features: list, topology_dataset: bool = False,
):
    """
    ...
    for workspace in rw_sde, ro_sde:
        ...
        with arcpy.EnvManager(workspace=workspace):
            ...
            features_ = arcpy.ListFeatureClasses()
    ...
    # add_features = list(set([".".join(x.split(".")[1:]) for x in add_features if "." in x]))

    if not add_features:
        raise IndexError(f"Did not find any features to add to replica.")
